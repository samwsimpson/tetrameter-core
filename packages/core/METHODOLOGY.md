...
| `grid.*` (resolution) | 2026.08.3 → 2026.08.4 | −19% to −91% for Vercel-hosted callers | Completes the row above. Vercel's region codes are short and provider-specific — `iad1` is not a country code and resolves by no other route — so Vercel-hosted callers still read 475. `iad1` is Vercel's default, making it the most common region code a caller can send. 18 Vercel regions added; `iad1` → US 384, `cdg1` → France 41. Vercel sets `VERCEL_REGION` at runtime, so no configuration is needed to supply it. |
